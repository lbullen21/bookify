...
- Recent Listening: ${userProfile.recentTracks?.map((t: { track: { name: string; artists: Array<{ name: string }> } }) => `${t.track.name} by ${t.track.artists[0].name}`).slice(0, 3).join(', ') || 'Not available'}
...
Keep responses under 150 words and include 2-3 specific book suggestions when possible.`;
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ 
        error: 'Sorry, I\'m having trouble connecting to the recommendation engine. Please try again!' 
      }, { status: 500 });
    }
